...
impl<'a, 'b> PartialEq<MerkleNode<'b>> for MerkleNode<'a> {
    fn eq(&self, other: &MerkleNode<'b>) -> bool {
        match (self, other) {
            (MerkleNode::Leaf(v1, d1), MerkleNode::Leaf(v2, d2)) if v1 == v2 && d1 == d2 => true,
            (MerkleNode::Extension(v1, d1), MerkleNode::Extension(v2, d2))
                if v1 == v2 && d1 == d2 =>
            {
                true
            }
            (MerkleNode::Branch(v1, d1), MerkleNode::Branch(v2, d2)) if v1 == v2 && d1 == d2 => {
                true
            }
            _ => false,
        }
...
impl<'a> MerkleNode<'a> {
    pub fn fast_decode_with_header(
        ...
        let node = if nibble::is_list_consume_rlp(
            &mut &buf[..h.payload_length],
            2, // len
        ) {
            // check if rlp_list.len() == 2 (leaf or extension)
            let NibblePair(nibble, typ) = fastrlp::Decodable::decode(buf)?;
            match typ {
                NibbleType::Leaf => MerkleNode::Leaf(nibble, fastrlp::Decodable::decode(buf)?),
                NibbleType::Extension => {
                    MerkleNode::Extension(nibble, fastrlp::Decodable::decode(buf)?)
                }
            }
        } else {
...
mod tests {
    ...
    fn decode_genesis_state() {
        ...
        let decoded_node: MerkleNode = fastrlp::Decodable::decode(&mut &*buffer).unwrap();
        println!("{:?}", decoded_node);
        check_roundtrip(&decoded_node);
